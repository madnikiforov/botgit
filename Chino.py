...
Owner = ["u9ab37772b2842c2b0734a0714c69f0cb"]
owner = ("u9ab37772b2842c2b0734a0714c69f0cb")
admin = ["u9ab37772b2842c2b0734a0714c69f0cb"]
#==============================================================================#
...
settings = {
    ...
    "autoAddMessage": "This is Bot Public By X̡̱̻n͓͡o̹̦ͬo̼̍̕b̞̐͝ Ft.Dͨͭ͘ö̸́̈ḻ̥̇p̖͖̈́h̢͂ͥi̙ͪ̾ń̸̬\n\nThanks @!,  for add me as a friends\n\nX̡̱̻n͓͡o̹̦ͬo̼̍̕b̞̐͝ Gans In Here http://line.me/ti/p/2fX-rPF8EL",
    "commentPost": "Autolike by X̡̱̻n͓͡o̹̦ͬo̼̍̕b̞̐͝\nhttp://line.me/ti/p/2fX-rPF8EL\n\nSupport by Team DFB Bot",
    "postMessage": "Auto Like By Me >.<\nhttp://line.me/ti/p/2fX-rPF8EL\n\n{\__/} \n( • - •) \n/ >🍪 mau biskuit? \n{\__/} \n( • - •) \n🍪< \ beli sendiri lah\n\n{\__/}\n( • - •)\n/ >👩🏻 Mau cewe ?\n{\__/}\n( • - •)\n👩🏻< \ W aja nikung temen\n\n{\_/}\n( •_•)\n/>📱Mau iPhoneX ?\n{\_/}\n( •_•)\n📱<\Kerja gblk\n\n{\__/} \n( • - •) \n/ >💰 mau line credit? \n{\__/} \n( • - •) \n💰< \ kumpulin sendiri lah\n\n{\_/}\n(• - •)\n/>💔 . mau?\n{\_/}\n(• - •)\n/>💔 . Ambil aja gapapa",
    "autoLike": False,
    "wcMsg":False,
    "botMute": [],
    "botOff": [],
...
def sendAnu(to, text="", mids=[]):
    ...
    chino.sendReplyMessage(msg.id, to, textx, {'AGENT_NAME':'「Self Message」', 'AGENT_LINK': 'http://line.me/ti/p/~bacbac.id', 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + chino.getProfile().picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
...
def sendMentionx(to, text="", mids=[]):
    ...
    if "@!" in text:
        ...
        arr.append(arrData)
        textx += mention + str(text)
    chino.sendReplyMessage(msg.id, to, textx, {'AGENT_NAME':'「  Sayang-Bot  」', 'AGENT_LINK': 'http://line.me/ti/p/%40gjp6777h', 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + chino.getProfile().picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
                    ...
                    if msg.contentType == 0:
                        if text is None:
                            ...
                            if cmd == "ahelp":
                                ...
                                chino.sendReplyMessage(msg.id, to, "==== Help ====\nMe\nGroupcreator\nGroupinfo\nReviewhp [nama hp]\nCinema\nGift\nSider [on/off]\nAnimequote\ncheckweather [lokasi]\ncheckpraytime [lokasi]\nartisifatnama [nama]\nkeahlian [nama]\nkemiripan [nama]\nchatowner: [isi pesan]\nbyebot")
                                chino.sendReplyMessage(msg.id, to, "Semua perintah diketik tanpa tanda kurung")
                            elif cmd == "asettings": 
                                ...
                                chino.sendMessage(to, "「CHANGE VIDEO PROFILE」\nKirim video")
                                ...
                                try:
                                    arr = []
                                    bagas = "u9ab37772b2842c2b0734a0714c69f0cb"                                   
                                    ...
                                    ret_ += "\n╰───[ Finish ]"
                                    sendMention(to, ret_, [chinoMid, bagas])
                                except Exception as error:
                                    	chino.sendReplyMessage(msg.id, to, str(error))

                            elif cmd.startswith("agbc "):
                            	if msg._from in Owner:
                                    sep = text.split(" ")
...
# Pembatas Script #                                
                            	...
                            	if msg._from in Owner:
                                 chino.sendContact(to, "u6347d96f19332503ee90b48d630ec27e',")                                 
                                    ...
                                    chino.sendReplyMessage(msg_id, to, None, contentMetadata={'mid': msg._from}, contentType=13)
                                    path = chino.getContact(msg.contentMetadata["mid"]).picturePath
#                                chino.generateReplyMessage(msg.id)
 #                               contact = chino.getContact(sender)        
  #                              image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
   #                             chino.sendMention(to, '[ Profile ]\n@!   ',[sender])
    #                            chino.sendMessage(to, "Bagas",contentMetadata={'vCard': 'BEGIN:VCARD\r\nVERSION:3.0\r\nPRODID:ANDROID 8.13.3 Android OS 4.4.4\r\nFN:\\Bagas Gans Here'+'\nTEL;TYPE=mobile:'+chino.getContact(sender).statusMessage+'\r\nN:?;\\,\r\nEND:VCARD\r\n', 'displayName': chino.getContact(sender).displayName},contentType=13)                                
#Self
                            elif cmd == "amymid":
                                chino.sendReplyMessage(msg.id, to, "[ MID ]\n{}".format(sender))
                                ...
                                contact = chino.getContact(sender)
                                chino.sendImageWithURL(to,"http://dl.profile.line-cdn.net/{}".format(contact.pictureStatus))
                                
                            elif cmd == "amyvideoprofile":
                                contact = chino.getContact(sender)
                                chino.sendVideoWithURL(to,"http://dl.profile.line-cdn.net/{}/vp".format(contact.pictureStatus))
...
#Steal#
                            ...
                            elif cmd.startswith("astealname "):
                                if 'MENTION' in msg.contentMetadata.keys()!= None:
                                    names = re.findall(r'@(\w+)', text)
                                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                                    mentionees = mention['MENTIONEES']
                            ...
                            elif cmd.startswith("astealpicture"):
                                if 'MENTION' in msg.contentMetadata.keys()!= None:
                                    names = re.findall(r'@(\w+)', text)
                                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                                    mentionees = mention['MENTIONEES']
                            ...
                            elif cmd.startswith("astealvideoprofile "):
                                if 'MENTION' in msg.contentMetadata.keys()!= None:
                                    names = re.findall(r'@(\w+)', text)
                                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                                    mentionees = mention['MENTIONEES']
                                    ...
                                    for ls in lists:
                                        ...
                                        chino.sendVideoWithURL(to, str(path))
                                        
                            elif cmd.startswith("astealcover "):
                                if chino != None:
                                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                                        names = re.findall(r'@(\w+)', text)
                                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                                        ...
                                        for ls in lists:
                                            ...
                                            path = str(channel)
                                            chino.sendImageWithURL(msg.id, to, str(path))
#Group#
...
# Pembatas Script #
                            elif cmd.startswith("achangename:"):
                            	if msg._from in Owner:
                                    sep = text.split(" ")
                            ...
                            elif cmd.startswith("achangebio:"):
                            	if msg._from in Owner:
                                    sep = text.split(" ")
                                    string = text.replace(sep[0] + " ","")
                                    if len(string) <= 500:
...
#MEDIA#                           
                                ...
                                try:
                                    ...
                                    hasil += "\nJudul : "+str(c)
                                    hasil += "\nLyric : "+str(f)
                                    chino.sendImageWithURL(to,g)
                                    chino.sendAudioWithURL(to,e)
                                    chino.sendMessage(to,hasil)
                                except Exception as error:
                                    chino.sendMessage(to, "error\n" + str(error))
                                    logError(error)
                                ...
                                try:
                                     ...
                                     me = best.url
                                     chino.sendReplyMessage(msg.id, to, "Send Video On Process ✈️")
                                ...
                                try:
                                    ...
                                    chino.sendMessage(to,a)
                                except Exception as e:
                                    chino.sendMessage(to, str(e))
                                ...
                                try:
                                    ...
                                    r = requests.get("http://api.farzain.com/yt_download.phFWp?id={}&apikey=hHu5Tv54ttrJy7Sh4zU4ANz".format(query))
                                    data = r.text
                                    data = json.loads(data)
                                ...
                                try:
                                    ...
                                    chino.sendMessage(to, "Porn Not Found !")
                                ...
                                if data["hot_threads"] != []:
                                    ...
                                    for news in data["hot_threads"]:
                                          ...
                                          hasil += "\n"
                                    chino.sendMessage(to, str(hasil))      
                                ...
                                if len(cond) == 1:
                                    ...
                                    chino.sendMessage(to, str(hasil)+ret_)
                                elif len(cond) == 2:
                                    no = int(cond[1])
                                    if no <= len(data["result"]):                                       
                                        gambar = data["result"][no - 1]
                                        chino.sendMessage(to, "Judul : "+str(gambar["judul"]))
                                        chino.sendImageWithURL(to, str(gambar["link"]))                                          
                                ...
                                try:                                   
                                    ...
                                    if data["content"] != []:
                                        ...
                                        b = len(items)
                                        chino.sendImageWithURL(to, str(path))
                                        chino.sendMessage(to,"Art #%s from #%s." %(str(a),str(b)))
                                ...
                                try:
                                     ...
                                     chino.sendImageWithURL(to, foto)
                                except Exception as e:
                                     chino.sendMessage(msg.to, str(e))
                                ...
                                try:
                                     ...
                                     r = requests.get("https://rest.farzain.com/api/cuaca.php?apikey=vZRHv2Fptb7CX4OgfHRaLMi6D&id={}".format(location), timeout=3)
                                     data = r.text
                                     data = json.loads(data)
                                ...
                                cond = query.split("|")
                                search = str(cond[0])
                                result = requests.get("http://api.farzain.com/gsmarena/gsmarena_search.php?id={}&apikey=hHu5Tv54tFWtrJy7Sh4zU4ANz".format(str(search)))
                                data = result.text
                                data = json.loads(data)
                                if len(cond) == 1:
                                    num = 0
                                    ...
                                    if num <= len(data):
                                        music = data[num - 1]
                                        result = requests.get("http://api.farzain.com/gsmarena/gsmarena_info.php?apikey=hHu5Tv54tFWtrJy7Sh4zU4ANz&id={}".format(str(music["url"])))
                                        data = result.text
                                        data = json.loads(data)
                                        if data != []:
                                            ret_ = "╭──「 Handphone 」"
                               ...
                               chino.sendReplyMessage(msg.id, to, str(hasil))
	                            ...
	                            sep = text.split(" ")
	                            search = text.replace(sep[0] + " ","")
	                            url = requests.get("https://rest.farzain.com/api/libur.php?name={}&apikey=hHu5Tv54tFWtrJy7Sh4zU4ANz".format(str(search)))
	                            data = url.text
	                            ...
	                            sep = text.split(" ")
	                            search = text.replace(sep[0] + " ","")
	                            url = requests.get("https://rest.farzain.com/api/ahli.php?name={}&apikey=hHu5Tv54tFWtrJy7Sh4zU4ANz".format(str(search)))
	                            data = url.text
	                            data = json.loads(data) #Ribetnya
	                            anu = data["result"]["result"]
	                            foto = data["result"]["image"]
	                            ...
	                            sep = text.split(" ")
	                            search = text.replace(sep[0] + " ","")
	                            url = requests.get("https://rest.farzain.com/api/mirip.php?name={}&apikey=hHu5Tv54tFWtrJy7Sh4zU4ANz".format(str(search)))
	                            data = url.text
	                            data = json.loads(data)
	                            anu = data["result"]["result"]
	                            foto = data["result"]["image"]
